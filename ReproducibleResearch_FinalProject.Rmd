...
Harm to population health was quantified through the total number of deceased and injured human beings, caused by each event throughout recorded history (one standalone event being more harmful than anoother one, by this standard, doesn't make it more harmful).  For that purpose, variables 'FATALITIES' and 'INJURIES' were used as total numbers of each type of harm, for each event.

### Measuring economic damage

Economic damage was quantified calculating the total of US Dollars lost due to damaged properties or crops, caused by each event throughout recorded history.  Variables 'PROPDMG' and 'CROPDMG' (dimensions of total damage to properties and crops respectivelly), plus 'PROPDMGEXP' and 'COPDMGEXP' (magnitude of the previous dimensions).  Detailed calculations in Appendix 2.
...
In order to quantify costs, as mentioned in Appendix 1, unique numeric variables for property and crop damage (ie: one '1.000' variable instead of '1' and 'K' variables) were created as follows:
```{r}
datafinal <- mutate(datafinal, PROPDMGVAL = if_else(PROPDMGEXP == "K", PROPDMG * 1000, if_else(PROPDMGEXP == "M", PROPDMG * 1000000, if_else(PROPDMGEXP == "B", PROPDMG * 1000000000, 0))), CROPDMGVAL = if_else(CROPDMGEXP == "K", CROPDMG * 1000, if_else(CROPDMGEXP == "M", CROPDMG * 1000000, if_else(CROPDMGEXP == "B", CROPDMG * 1000000000, 0))))
```

Additionally, a final variable that added up the total damage value ('PRPODMGVAL' + 'CROPDMGVAL'), was created as follows:
```{r}
datafinal <- mutate(datafinal, TOTALDMGVAL = PROPDMGVAL + CROPDMGVAL)
```

PENDIENTE REEMPLAZAR NAs PARA QUE LA SUMA ANTERIOR FUNCIONE.
