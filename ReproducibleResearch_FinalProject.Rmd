...
When applying a table view of all different types of 'EVTYPE' variables (expecting to obtain no more than a two-digit result), a humongous list was obtained, with almost 1.000 different values:
```{r}
head(table(datafinal$EVTYPE), 15)
```

Just out of the first 15, it was automatically concluded that no standardized values were contained within that variable (ie: 'beach erosion' written three different ways -including one misspelled version-).
